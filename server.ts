...
async function handleStreamingChat(message, ws, chatHistory, timeZone) {
  try {
    ...
    const classifierPrompt = `
        ...
        "B": Specific event search. The user explicitly asks for events, workshops, or classes about a specific topic, or implies a topic based on the recent chat history context. Example: "Yoga classes on Friday", "Sound healing events", "Are there any music concerts today?".
             CRITICAL: If the query refers to a treatment, therapy, class, workshop, active practice, or wellness session (such as massage, yoga, sound healing, pilates, or meditation), you MUST classify it as "B" (Specific Event Search) instead of "C", even if they only typed a single keyword like "massage", "yoga", "sound healing", or "meditation" without explicitly writing "class" or "event".
        ...
        "C": General information, facts, services, or conversational questions. Use this for places (e.g., "Matrimandir"), services (e.g., "bus service", "volunteering"), or broad topics. IF THE CURRENT QUERY IS AMBIGUOUS (and not an active practice/treatment like massage, yoga, sound healing, meditation), classify it as "C".
    ...
    console.log(`Search Query / Parameters: ${JSON.stringify({ searchQuery, filterDate, filterStartDate, filterEndDate, filterDay, filterTimeAfter })}`);
...
async function createServer() {
  ...
  app.post("/api/chat", express.json(), async (req, res) => {
    ...
    try {
        ...
        let bucket = "C", searchQuery = lastMessage, introText = "", filterDate = "", filterStartDate = "", filterEndDate = "", filterDay = "", filterTimeAfter = "";
        try {
            const parsed = JSON.parse(classRes.text || "{}");
            bucket = parsed.bucket || "C";
        ...
        if (bucket === "A") {
             ...
             const rawEventsArray = await searchAurovilleEvents(searchQuery, "specific", filterDay, filterDate, filterStartDate, filterEndDate, filterTimeAfter, true, tz);
